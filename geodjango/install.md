...
<u>**Note**</u>
* コマンドプロンプトを閉じると設定が無効になってしまうので、コマンドプロンプトを実行するたびに設定する必要がありますので、下記の用にbatファイルにしておくことをお勧めします。また、システムの環境変数に設定をすると毎回設定をする手間を省けます。
...
PostGISに管理画面にログイン権限のある管理ユーザのアカウントを作成します。
